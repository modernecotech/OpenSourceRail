...
if [[ ${2:-} == "--publish-reference" ]]; then
    echo "note: --publish-reference is now the default; writing to designs/" >&2
elif [[ $# -eq 2 ]]; then
...
"$REPO/scripts/engineering-toolchain.sh" --cities \
...
"$PYTHON" "$REPO/scripts/validate-city-simulation.py" \
    --scenario "$DESIGN_DIR/$SLUG.toml" --resilience
"$PYTHON" "$REPO/scripts/render-sim-screenshots.py" \
    --scenario "$DESIGN_DIR/$SLUG.toml"

echo "7) operations package → $DESIGN_DIR/operations/"
"$PYTHON" "$REPO/scripts/generate-qa-maintenance-data.py" \
    --design "$DESIGN_DIR/design.toml" \
    --scenario "$DESIGN_DIR/$SLUG.toml" \
    --out-dir "$DESIGN_DIR/operations"
"$PYTHON" "$REPO/scripts/generate-acceptance-evidence-report.py" \
...
"$PYTHON" "$REPO/scripts/generate-city-package-manifest.py" \
    --city-dir "$DESIGN_DIR"
