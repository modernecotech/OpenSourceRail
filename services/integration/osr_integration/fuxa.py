...
def _transport_id(package, asset):
    if package.get('fuxa_polling_scope', 'asset') == 'site':
        return 'osr-site-' + digest([asset['environment'],asset['city'],asset['site_id']])[:20]
    return asset['fuxa_device_id']
...
def deployment_manifest(packages, desired=None, *, workbench_url=DEFAULT_WORKBENCH_URL):
    ...
    for package in packages:
        ...
        rows.append(dict(city=package['city'], environment=package['environment'],
            ...
            devices=sorted({_transport_id(package,row) for row in package['equipment']}),
    ...
    for package in packages:
        ...
        for site in sites:
            ...
            for index, a in enumerate(a for a in package['equipment'] if a['site_id'] == site):
                device = a['fuxa_device_id']; transport = _transport_id(package,a); x, y = 30 + (index % 2) * 580, 95 + (index // 2) * (card_height + 20)
                tags = {}
                connection = transport + '__connection'
                system_tags[connection] = dict(id=connection, name=connection, type='number', sysType=1, memaddress=transport, init=0, daq={'enabled': False})
                connection_widget = 'VAL_' + digest(connection if transport==device else [device,connection])[:20]
                ...
                for row, (name, m) in enumerate(a['measurements'].items()):
                    ...
                    for suffix, dx, dy, dtype in [('', 270, 0, 'Double'), ('_quality', 390, 0, 'String'), ('_timestamp', 270, 23, 'String')]:
                        ...
                        variable = tid
                        items[wid] = dict(id=wid, type='svg-ext-value', name=tid, label='Value', property=dict(variable=tid, variableId=variable, variableSrc=transport, events=[], ranges=[], actions=visibility))
                        variables[variable] = dict(id=tid, name=tid, source=transport)
                ...
                path, identity = ('site-tags',a['site_id']) if package.get('fuxa_polling_scope','asset')=='site' else ('tags',a['asset_id'])
                endpoint=f'http://integration:8093/{path}/{a["city"]}/{a["environment"]}/{identity}'
                if transport not in devices:
                    devices[transport] = dict(id=transport,name=transport,type='WebAPI',enabled=True,polling=2000,
                        property=dict(getTags=endpoint),tags={})
                devices[transport]['tags'].update(tags)
            svg.append('</g></svg>')
