...
def project(packages):
    ...
    for package in packages:
        ...
        for site in sites:
            ...
            items, variables = {}, {}
            svg = ['<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="940"><g id="svgcontent">',
            ...
            for index, a in enumerate(a for a in package['equipment'] if a['site_id'] == site):
                device = a['fuxa_device_id']; x, y = 30 + (index % 2) * 580, 95 + (index // 2) * 395
                tags = {}
                ...
                svg += [f'<rect x="{x}" y="{y}" width="550" height="375" rx="12" fill="#eef5f7" stroke="#b5cbd0"/>',
                ...
                for n, rule in enumerate(a['alarms']):
                    ...
                    items[wid] = dict(id=wid, type='svg-ext-value', name=tid, label='Value', property=dict(variable=tid, variableId=variable, variableSrc=device, events=[], ranges=[], actions=visibility))
                    variables[variable] = dict(id=tid, name=tid, source=device)
                    svg += [f'<text x="{x+20}" y="{y+310+n*20}" font-size="14" fill="#a43228">{escape(rule["id"])} alarm (1 active)</text>', f'<g id="{wid}" type="svg-ext-value"><text id="text_{wid}" x="{x+280}" y="{y+310+n*20}" fill="#a43228">0</text></g>']
                url = 'http://127.0.0.1:8090/docs/lifecycle/?' + urlencode({'city': a['city'], 'environment': a['environment'], 'asset': a['asset_id']})
                svg.append(f'<a href="{escape(url, quote=True)}" target="_blank"><text x="{x+20}" y="{y+352}" fill="#166a70" font-size="14">Open asset, maintenance, engineering and history</text></a>')
            ...
            views.append(dict(id=view_id, name=f'{package["city"]} · {site} · {package["environment"]}', type='svg',
                              profile=dict(width=1200, height=940, bkcolor='#ffffff'), items=items, variables=variables, svgcontent=''.join(svg)))
    ...
    return dict(version='1.02', name='OpenSourceRail station supervision', devices=devices,
                ...
                hmi=dict(views=views, layout=dict(start=views[0]['id'], navigation=dict(mode='fix', type='inline', items=navigation),
                         header={}, showdev=True, loginonstart=True, show_connection_error=True)))
