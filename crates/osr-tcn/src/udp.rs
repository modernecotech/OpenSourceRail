...
pub struct UdpTcn {
    ...
    rx_buffer: BTreeMap<TopicId, VecDeque<(TrafficClass, Vec<u8>)>>,
...
impl UdpTcn {
        ...
        loop {
            match self.socket.recv_from(&mut buf) {
                Ok((n, _from)) => {
                    ...
                    let Some(class) = class_from_tag(buf[2]) else {
                        continue;
                    };
    ...
    pub fn recv_one<P: TcnPayload>(&mut self, topic: TopicId) -> Option<Result<P, TcnError>> {
        ...
        let (class, bytes) = queue.pop_front()?;
        if class != P::CLASS {
...
fn class_from_tag(tag: u8) -> Option<TrafficClass> {
    match tag {
        0 => Some(TrafficClass::Safety),
        1 => Some(TrafficClass::Control),
        2 => Some(TrafficClass::App),
        _ => None,
    }
}
