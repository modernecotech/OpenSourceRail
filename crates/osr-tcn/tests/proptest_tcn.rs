...
use osr_tcn::{MockTcn, TcnError, TcnPayload, TopicId, TopicRegistry, TrafficClass};
...
proptest! {
    ...
    /// T6: a topic's safety class is immutable after its first publish.
    #[test]
    fn t6_topic_class_is_immutable(value in any::<i64>(), text in "[a-z]{1,12}") {
        let mut tcn = MockTcn::new();
        let topic = safety_topic(&tcn);
        tcn.publish(topic, &SafetyMsg { value, ts: 0 }).unwrap();
        let result = tcn.publish(topic, &AppMsg { text, code: 0 });
        prop_assert_eq!(result, Err(TcnError::ClassMismatch));
        prop_assert_eq!(tcn.depth(topic), 1);
        prop_assert_eq!(tcn.drops(topic), 0);
    }

    /// T7: independent registries built from the same ordered input
    /// assign exactly the same IDs, including duplicate interns.
    #[test]
    fn t7_registry_mapping_is_deterministic(
        names in prop::collection::vec("[a-z]{1,8}(\\.[a-z]{1,8}){0,3}", 0..100),
    ) {
        let mut left = TopicRegistry::new();
        let mut right = TopicRegistry::new();
        let left_ids: Vec<_> = names.iter().map(|name| left.intern(name)).collect();
        let right_ids: Vec<_> = names.iter().map(|name| right.intern(name)).collect();
