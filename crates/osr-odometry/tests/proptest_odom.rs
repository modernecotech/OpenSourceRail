...
#[test]
fn saturated_uncertainty_is_not_known_for_atp() {
    let c = cal();
    let mut odom = init_at_offset(0);
    odom.position_uncertainty_mm = c.max_uncertainty_mm;
    assert!(odom.to_train_state_if_known(c.max_uncertainty_mm).is_none());

    odom.position_uncertainty_mm = c.max_uncertainty_mm - 1;
    assert!(odom.to_train_state_if_known(c.max_uncertainty_mm).is_some());
}
