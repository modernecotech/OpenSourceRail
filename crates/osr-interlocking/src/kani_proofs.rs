...
use osr_core::{
    ConsistDescriptor, Direction, EntryId, Network, Position, Section, SectionId, StaticTopology,
    StationId, TrackLine, TrackRef, TrainId,
};
...
fn three_section_network() -> StaticTopology<'static> {
...
fn tiny_network() -> StaticTopology<'static> {
    ...
    const LINES: [TrackLine<'static>; 1] = [TrackLine {
        forward_sections: &[SectionId::new(1000), SectionId::new(1001)],
        reverse_sections: &[SectionId::new(2000), SectionId::new(2001)],
        is_ring: false,
    }];
    const TOPOLOGY: StaticTopology<'static> = match StaticTopology::try_new(&LINES, &SECTIONS) {
        Ok(topology) => topology,
