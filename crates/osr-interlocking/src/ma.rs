...
pub fn compute_self_ma_from_state(
    ...
    let applicable_restrictions =
        collect_applicable_restrictions(state, head.track_ref, ma_end, now_ns);
...
fn collect_applicable_restrictions(
    ...
    // v1 simplification: return restrictions on either the from or to
    // section. Refining this to precise geometric overlap is M2-next.
    state
        .speed_restrictions
        .iter()
        .filter(|sr| {
            ...
            // Relevant section (v1 heuristic).
            sr.section == from.section || sr.section == to.section
        })
        .cloned()
        .collect()
}
