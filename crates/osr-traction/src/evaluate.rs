...
pub fn traction_evaluate(
    ...
    } else if !inputs.bms_contactor_closed || !inputs.enable_requested {
    ...
    let slipping_under_traction =
        slip_mmps > params.slip_threshold_mmps.max(1) && shaped_setpoint > 0;
    let sliding_under_regen = slip_mmps < -params.slip_threshold_mmps.max(1) && shaped_setpoint < 0;
    if slipping_under_traction || sliding_under_regen {
