...
//! chemistry is LFP on an 800 V-class per-car DC link.
...
//!   current. Returned in parts-per-thousand (0…1000). SoH uses a
//!   conservative equivalent-full-cycle throughput model with a
//!   configurable hot-cycle acceleration factor.
...
//! - **M7 (battery hazard is fail-safe):** an off-gas or external fire
//!   input inhibits charge and opens the car-pack contactor. Module/string
//!   identity is reported for service response; it does not imply that a
//!   series module can be electrically bypassed.
//!
...
pub use types::{ContactorState, FaultReason};
