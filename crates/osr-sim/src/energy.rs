...
//!
//! Scenarios without any `[[sites]]` run in "unlimited free charging" mode,
//! which preserves the earlier simulator behavior.
...
impl EnergySite {
    ...
    pub fn draw(&mut self, max_kwh: f32, dt_s: f32, grid_disabled: bool) -> f32 {
        ...
        let delivered = max_kwh - remaining;
...
pub struct EnergySystem {
    pub sites: HashMap<StationId, EnergySite>,
...
impl EnergySystem {
    ...
    /// Request charging energy at a station. Scenarios without `[[sites]]`
    /// return `max_kwh` unchanged (charging is unlimited).
