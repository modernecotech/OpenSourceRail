...
pub struct DispatchThrottle {
    ...
    /// Maximum effective headway used during the run.
...
impl DispatchThrottle {
    ...
    pub fn mark_dispatched(
        &mut self,
        key: &ThrottleKey,
        t_since_midnight: u32,
        published_headway_s: u32,
        effective_headway_s: u32,
    ) {
        if self.throttle_points.contains(key) {
            ...
            self.next_allowed.insert(*key, base + effective_headway_s);
            if effective_headway_s > published_headway_s {
                self.energy_adaptive_dispatches += 1;
                self.energy_adaptive_headway_added_s +=
                    u64::from(effective_headway_s - published_headway_s);
            }
            self.maximum_effective_headway_s =
                self.maximum_effective_headway_s.max(effective_headway_s);
        }
    }
