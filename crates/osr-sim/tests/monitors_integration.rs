...
//! 2. Vigilance (driver alerter timeout).
//! 3. Fire safety (smoke / heat in battery, traction, HVAC bays).
//! 4. Derailment (lateral accel + tilt + vertical shock, 2oo2).
//! 5. Driver emergency plunger (synthetic `false` in the shadow).
//!
//! Any trip in (1)–(4) signals either a regression in the monitor
...
fn nominal_run_has_no_monitor_trips() {
    ...
    assert_eq!(
        ob.total_vigilance_trip_ticks, 0,
        "vigilance tripped under nominal service (auto-acknowledge should prevent this)"
    );
...
fn monitors_run_every_traveling_tick() {
    ...
    // called, fire/derail/vigilance state would never have
