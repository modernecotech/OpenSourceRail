...
//! - **repeatable 500 kWh stationary-LFP battery modules** — buffering
//! - **Grid-tie inverter** — bidirectional import / export
//! - **One shared 500 kW DC/DC cabinet** — interlocked between up to two
//!   platform contacts
//!
...
pub struct EnergySiteParams {
    /// Shared cabinet power; not a per-contact value.
    pub pad_max_w: u32,
    pub pad_max_current_a: u32,
    pub train_bus_voltage_v: u32,
...
impl EnergySiteParams {
    ...
    pub fn default_samawah() -> Self {
        Self {
            pad_max_w: 500_000,
            pad_max_current_a: 825,
            train_bus_voltage_v: 650,
            contact_count: 2,
...
/// Deterministic, work-conserving split of one cabinet budget between two
/// platform contacts. If both requests exceed the cabinet, each receives a
/// proportional share; unused capacity on one contact flows to the other.
#[derive(Copy, Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
...
#[must_use]
pub fn allocate_shared_contacts(
    request_a_w: u32,
    request_b_w: u32,
    budget_w: u32,
) -> ContactAllocation {
    let total = u64::from(request_a_w) + u64::from(request_b_w);
    if total == 0 || budget_w == 0 {
        return ContactAllocation {
            contact_a_w: 0,
            contact_b_w: 0,
        };
    }
    if total <= u64::from(budget_w) {
        return ContactAllocation {
            contact_a_w: request_a_w,
            contact_b_w: request_b_w,
        };
    }
    let a = (u64::from(budget_w) * u64::from(request_a_w) / total) as u32;
    ContactAllocation {
        contact_a_w: a,
        contact_b_w: budget_w - a,
    }
}
...
pub fn energy_site_evaluate(
    ...
    let current_limited_w = params
        .pad_max_current_a
        .saturating_mul(params.train_bus_voltage_v);
    let shared_cabinet_limit_w = params.pad_max_w.min(current_limited_w);
    let pad_request = inputs.pad_request_w.min(shared_cabinet_limit_w);
...
mod tests {
    ...
    fn train_at_pad_drains_battery_and_grid() {
        ...
        let out = energy_site_evaluate(&i, &EnergySiteParams::default_samawah());
        assert_eq!(out.to_pad_w, 500_000);
        assert_eq!(out.battery_discharge_w, 400_000);
    ...
    #[test]
    fn two_contacts_share_one_cabinet() {
        let a = allocate_shared_contacts(500_000, 500_000, 500_000);
        assert_eq!(a.contact_a_w + a.contact_b_w, 500_000);
        assert_eq!(a.contact_a_w, 250_000);
    }

    #[test]
    fn current_limit_is_enforced() {
        let i = inputs(0, 500_000, 500, true);
        let mut p = EnergySiteParams::default_samawah();
        p.train_bus_voltage_v = 500;
        let out = energy_site_evaluate(&i, &p);
        assert_eq!(out.to_pad_w, 412_500);
    }
}
