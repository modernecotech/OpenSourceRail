...
pub enum TriggerReason {
    ...
    Overspeed,
    /// Measured speed is above an applicable movement-authority speed
    /// restriction.
    SpeedRestriction,
}
...
pub fn atp_evaluate(
    ...
    let restriction_envelope = ma
        ...
        .iter()
        .map(|sr| sr.max_speed_mmps.max(0))
        .min()
        .and_then(|v| i32::try_from(v).ok())
        .unwrap_or(i32::MAX);
    let envelope = brake_envelope.min(restriction_envelope);
    let limiting_restriction = restriction_envelope < brake_envelope;
    ...
    if margin < -OVERSPEED_EMERGENCY_MARGIN_MMPS {
        AtpOutcome {
            command: BrakeCommand::Emergency,
            reason: if limiting_restriction {
                TriggerReason::SpeedRestriction
            } else {
...
mod tests {
    ...
    use osr_interlocking::{
        MovementAuthority, RestrictionReason, SpeedRestriction, MA_VALIDITY_WINDOW_NS,
    };
    ...
    #[test]
    fn speed_restriction_caps_envelope() {
        let net = net_3_sections();
        let consist = ConsistDescriptor::reference_3car();
        let mut state = state_stopped_at(1000, 0);
        state.speed_mmps = 10_000;
        let mut ma = nominal_ma(1001, 1_000_000, 0);
        ma.applicable_restrictions.push(SpeedRestriction {
            section: SectionId::new(1000),
            from_offset_mm: 0,
            to_offset_mm: 1_000_000,
            max_speed_mmps: 8_000,
        ...
        let out = atp_evaluate(&state, &ma, &consist, &net, 0);
        assert!(out.is_emergency(), "{out:?}");
        assert_eq!(out.reason, TriggerReason::SpeedRestriction);
        assert_eq!(out.envelope_mmps, Some(8_000));
    }
