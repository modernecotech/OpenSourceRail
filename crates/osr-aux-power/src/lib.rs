...
//!
//! Manages the three aux-rails a light-metro vehicle runs on top of
//! the traction pack:
//!
//! | Rail | Loads | Priority |
//! |------|-------|----------|
...
//! | **400 V AC** | HVAC, air compressors, other high-power comfort loads | Lowest |
//!
...
//!
//! - **SoC ≤ `shed_400v_ppt`** (e.g., 300 ppt = 30 %) → drop the
//!   400 V AC rail. HVAC and compressors off; passenger comfort
...
impl Rail {
    pub const ALL: [Rail; 3] = [Rail::V24, Rail::V110, Rail::V400];
}
...
pub struct AuxInputs {
    ...
    pub v400_enable_request: bool,
}

#[derive(Copy, Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct AuxParams {
    /// SoC at or below which the 400 V rail sheds (ppt).
    pub shed_400v_ppt: u16,
...
pub fn aux_evaluate(prev: &AuxState, inputs: &AuxInputs, params: &AuxParams) -> AuxOutput {
    ...
    if inputs.v400_over_temp || inputs.v400_drive_fault {
        current_faults.insert(Rail::V400);
    }
    ...
    let v400_enabled = contactor_ok
        && !rail_faulted(Rail::V400)
        && inputs.v400_enable_request
        && soc > params.shed_400v_ppt;

    // Shedding is any time at least one of 110 V / 400 V is down due
    // to SoC while the contactor is up and there's no fault.
    let load_shed_active = contactor_ok
        && (((soc <= params.shed_400v_ppt) && !rail_faulted(Rail::V400))
...
mod tests {
    ...
    fn high_soc_all_rails_enabled() {
        ...
        assert!(out.v400_enabled);
        assert!(!out.load_shed_active);
    }

    #[test]
    fn medium_soc_sheds_400v() {
        let out = aux_evaluate(
            &AuxState::default(),
            &nominal_inputs(250), // below 300 shed_400v
            &AuxParams::light_metro_default(),
        );
        assert!(out.v24_enabled);
        assert!(out.v110_enabled);
    ...
    #[test]
    fn low_soc_sheds_110v_and_400v() {
        let out = aux_evaluate(
            &AuxState::default(),
            &nominal_inputs(100),
            &AuxParams::light_metro_default(),
    ...
    fn fault_cooldown_latches() {
        ...
        let mut i = nominal_inputs(800);
        i.v400_drive_fault = true;
        let state = aux_evaluate(&AuxState::default(), &i, &p).state;
        assert!(state.fault_until_ns[Rail::V400 as usize].is_some());
        ...
        assert!(!out.v400_enabled);
        assert!(out.state.faults.contains(Rail::V400));
