...
//!
//! The direct-HV DC comfort branch (from [`osr_aux_power`]) powers the
//! compressor and heater drives. When that branch is off (either faulted or load-shed
...
pub struct HvacInputs {
    ...
    pub setpoint_dc: i16,
    /// Direct-HV DC comfort branch available per RFC 0021.
...
pub enum HvacMode {
    ...
    Cooling,
    Heating,
    /// Direct-HV branch down; degraded fans-only mode.
    Reduced,
}
...
pub fn hvac_evaluate(prev: &HvacState, inputs: &HvacInputs, params: &HvacParams) -> HvacOutput {
    ...
    // Reduced mode when the direct-HV branch is unavailable.
    if !inputs.direct_hv_enabled {
        return HvacOutput {
            state: HvacState { integral_ppt: 0 },
