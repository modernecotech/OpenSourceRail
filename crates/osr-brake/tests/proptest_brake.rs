...
fn arb_inputs() -> impl Strategy<Value = BrakeInputs> {
    (
        ...
        .prop_map(
            |(cmd, fire, derail, remote_assist, obstacle, park, meas, wheel, regen)| BrakeInputs {
                atp_command: cmd,
                fire_emergency: fire,
                derailment_emergency: derail,
                remote_assist_emergency: remote_assist,
...
proptest! {
    ...
    fn b2_emergency_union(i in arb_inputs()) {
        ...
        prop_assert_eq!(out.emergency_sources.remote_assist, i.remote_assist_emergency);
...
proptest! {
    ...
    fn b3_emergency_completeness(i in arb_inputs()) {
        ...
        let out = brake_evaluate(&i, &p);
        if out.is_emergency() {
            let command = p.min_friction_emergency_ppt.min(1000);
            let reduction =
                u32::from(command) * u32::from(p.wsp_reduction_ppt.min(1000)) / 1000;
            let floor = u32::from(command).saturating_sub(reduction) as u16;
            prop_assert!(
            ...
            prop_assert_eq!(out.friction_command_before_wsp_ppt, command);
        }
    }
}
