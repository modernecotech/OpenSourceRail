...
fn arb_inputs() -> BrakeInputs {
    ...
    BrakeInputs {
        ...
        remote_assist_emergency: kani::any(),
...
fn kani_b2_emergency_union() {
    ...
    let any_source = matches!(i.atp_command, BrakeCommand::Emergency)
        ...
        || i.remote_assist_emergency
        || i.obstacle_emergency;
    ...
    assert!(out.emergency_sources.remote_assist == i.remote_assist_emergency);
    assert!(out.emergency_sources.obstacle == i.obstacle_emergency);
}
...
fn kani_b3_emergency_completeness() {
    ...
    // Force Emergency via authenticated remote assistance while the
    // other fields stay symbolic.
