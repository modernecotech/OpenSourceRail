...
pub fn brake_evaluate(inputs: &BrakeInputs, params: &BrakeParams) -> BrakeOutput {
    ...
    let sources = EmergencySources {
        ...
        remote_assist: inputs.remote_assist_emergency,
...
fn emergency_output(
    ...
    let friction_command = params.min_friction_emergency_ppt.min(1000);
...
fn wsp_modulate(
    ...
    // Reduce the commanded friction by `wsp_reduction_ppt` parts per
    // thousand of the commanded effort. This preserves monotonicity:
    // higher commanded → higher actual.
    let reduction =
        u32::from(friction_command_ppt) * u32::from(params.wsp_reduction_ppt.min(1000)) / 1000;
...
mod tests {
    ...
    #[test]
    fn remote_assist_escalation_alone_trips() {
        let mut i = nominal_inputs();
        i.remote_assist_emergency = true;
        let p = BrakeParams::light_metro_default();
        let out = brake_evaluate(&i, &p);
        assert!(out.is_emergency());
        assert!(out.emergency_sources.remote_assist);
        assert!(!out.emergency_sources.atp);
    }
    ...
    fn wsp_activates_on_slide_and_subtracts() {
        ...
        assert_eq!(out.friction_effort_ppt, 480);
    }

    #[test]
    fn bench_emergency_floor_is_honoured() {
        let mut i = nominal_inputs();
        i.atp_command = BrakeCommand::Emergency;
        i.measured_speed_mmps = 10_000;
        i.wheel_speed_mmps = 10_000;
        let mut p = BrakeParams::light_metro_default();
        p.min_friction_emergency_ppt = 500;
        let out = brake_evaluate(&i, &p);
        assert_eq!(out.friction_command_before_wsp_ppt, 500);
