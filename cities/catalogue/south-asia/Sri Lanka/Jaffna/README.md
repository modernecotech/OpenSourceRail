# Jaffna — Urban Rail Network

**Country:** LK · **Population:** 600,000 · [National brief](../NATIONAL-BRIEF.md)

This page contains only Jaffna-specific results. Shared routing, service, energy, civil, cost, finance, QA and validation methods are defined once in the [deployment planning reference](../../../../../docs/deployment-planning-reference.md).

> [!IMPORTANT]
> **Foreign-capital advantage:** against the default equivalent foreign-turnkey sensitivity, this local plan avoids **$707 M (86.0%) of external capital** and **$887 M of external interest**. Capital plus saved interest totals **$1.59 bn**. See the common reference for interpretation and limitations.

Auto-planned by the OpenSourceRail design pipeline from the controlled city catalogue, source-locked geospatial inputs and shared templates.

## Network

![Jaffna rail network on OpenStreetMap](jaffna-network-map.png)

| Local measure | Value |
|---|---:|
| Lines / unique stations / interchanges | 3 / 21 / 1 |
| Route length | 53.4 km double track |
| Coverage / transfer reachability | 69.6% / 100% |
| Estimated station catchment | 417,600 residents |
| Service span / peak headway | 05:30–02:00 / 3 min |
| Fleet | 131 × 3-car `light-metro-3car` trainsets (118 peak revenue) |
| Peak network throughput | 43,200 passengers/hour |
| Practical service capacity | 401,760 passenger-trips/day |
| Annual paid-trip planning range | 73.3–117.3 M |

### Lines

| Line | Length | Stations | Trainsets | Termini |
|---|---:|---:|---:|---|
| line-1 | 13.9 km | 7 | 36 | S Mid ↔ N Mid |
| line-2 | 22.5 km | 7 | 54 | SE Mid ↔ W Outer |
| line-3 | 17.1 km | 7 | 41 | NE Outer ↔ S Mid |
| **Total** | **53.4 km** | **21 unique** | **131** | |

## Energy

| Local measure | Value |
|---|---:|
| Scheduled service | 1,395 one-way journeys / 24,837 train-km/day |
| Annual traction demand | 117.5 GWh |
| Station/depot PV / storage | 10.4 MW / 49.0 MWh |
| Aggregate charging power | 9.5 MW |
| Dedicated solar plant | 65.2 MW |
| Residual grid/PPA import | 0.0 GWh/yr |
| Worst powered-stop gap | line-2: 10.8 km / 81 kWh |
| Lowest traversal charging margin | line-3: 56 kWh |

## Capital And Funding

| Local CAPEX bucket | Planning value |
|---|---:|
| Civil works | $152 M |
| Stations | $96 M |
| Depots | $8.0 M |
| Rolling stock | $118 M |
| Dedicated solar plant | $52 M |
| Residual train control | $2.7 M |
| Charging microgrids | $2.1 M |
| EPC / project services | $26 M |
| **Total city programme** | **$457 M** |

| Local funding measure | Planning value |
|---|---:|
| Imported / external capital | $115 M (25.1%) |
| Domestic / local capital | $342 M (74.9%) |
| Annual public construction commitment | $52 M / yr for 7 years |
| Annual post-grace debt service | $44 M / yr |
| External capital saved vs default turnkey sensitivity | $707 M |
| Capital + lifetime external interest saved | $1.59 bn |
| Annual OPEX | $12 M / yr |

## Local Evidence

**Evidence refresh required.** Retained passing results below are unverified.
The strict README generator rejected the evidence: engineering/simulation/validation-summary.json describes scenario SHA-256 2e13f6a6f63ea7579793e8a640c5c3d36d3f99727a16f8b18cf6531e51e30d9d, but jaffna.toml is fe86ce146489b7fd2a00d886dbc59666b01474815369e4def2af772889eab291; rerun and update the validation evidence. This audit view does not accept or replace the retained solver results.

| Package | Current status | Evidence |
|---|---|---|
| Finance | unverified | [`summary.json`](engineering/finance/summary.json) |
| Native simulation + degraded cases | unverified | [`validation-summary.json`](engineering/simulation/validation-summary.json) |
| SUMO timetable | unverified | [`summary.json`](engineering/sumo/summary.json) |
| Independent OSR/SUMO running-time cross-check | missing/fail; junction/authority gate open | not generated |
| GIS package | unverified | [`summary.json`](engineering/gis/summary.json) |
| Grid/charging/solar | missing/fail; 7 findings | [`summary.json`](engineering/energy/summary.json) |
| Operations, QA and maintenance | 258 assets / 1,239 tasks | [`jaffna-operations-manifest.json`](operations/jaffna-operations-manifest.json) |

## Local Files And Regeneration

| File | Local role |
|---|---|
| [`design.toml`](design.toml) | Authoritative generated city design |
| [`jaffna.toml`](jaffna.toml) | Expanded simulator scenario |
| [`jaffna.corridor.geojson`](jaffna.corridor.geojson) | GIS corridor and stations |
| [`jaffna.design-quality.yaml`](jaffna.design-quality.yaml) | Coverage, source and civil-quality gates |

```bash
tools/automation/regenerate-city.sh jaffna
```
