...
from .sidecar import read_sidecar, validate_sidecar
...
def emit_toml(aln: Alignment, meta: Meta, sidecar: dict | None = None) -> str:
    ...
    out.append(f"# Alignment name: {aln.name.replace(chr(10), ' ').replace(chr(13), ' ')}")
    out.append(f"# Total length: {aln.length_m:.2f} m")
    out.append("")
    out.append("[meta]")
    for key, value in (("schema_version", meta.schema_version), ("line_id", meta.line_id),
                       ("design_date", meta.design_date), ("surveyor", meta.surveyor),
                       ("preset", meta.preset), ("consist", meta.consist),
                       ("crs", meta.crs), ("units", meta.units)):
        out.append(f'{key:14} = {json.dumps(value, ensure_ascii=False)}')
    out.append(f"is_ring        = {str(meta.is_ring).lower()}")
    if sidecar is not None:
        out.append(f'vertical_datum = {json.dumps(sidecar["source"]["vertical_datum"], ensure_ascii=False)}')
    out.append("")
    out.append("# ----------------------------------------------------------")
    ...
    if sidecar is not None:
        out.extend(["", "# Explicit companion data; validation is not engineering approval."])
        for section, fields in (
            ("civil", ("from_station_m", "to_station_m", "class")),
            ("station", ("id", "station_m", "platform_length_m")),
            ("cant", ("from_station_m", "to_station_m", "max_cant_mm", "transition_in_m", "transition_out_m")),
        ):
            for row in sidecar[section]:
                out.extend(["", f"[[{section}]]"])
                out.extend(f"{key} = {json.dumps(row[key], ensure_ascii=False, allow_nan=False)}" for key in fields)
        out.extend(["", "[provenance]", 'kind = "landxml-with-explicit-sidecar"'])
        for key, value in {**sidecar["source"], "sidecar_sha256": sidecar["sha256"]}.items():
            out.append(f"{key} = {json.dumps(value, ensure_ascii=False)}")
        out.extend(["deployment_release_ready = false", ""])
        return "\n".join(out)
    out.append("")
    out.append("# ----------------------------------------------------------")
    ...
    for s in aln.stations:
        ...
        out.append("[[station]]")
        out.append(
            f'id                = {json.dumps(s.placeholder_id, ensure_ascii=False)}   # SOURCE NAME: map to design.toml before release'
        )
...
def convert(
    ...
    *,
    ...
    if (sidecar_path is None) != (design_path is None):
        raise ValueError("--sidecar and --design must be supplied together")
    sidecar = read_sidecar(sidecar_path, xml_path, design_path, meta) if sidecar_path is not None else None
    ...
    # Legacy imports retain the first-alignment behavior. An explicit sidecar
    # selects exactly one named alignment instead of guessing by file order.
    aln_el = alignments_el.find(_tag("Alignment"))
    if sidecar is not None:
        matches = [el for el in alignments_el.findall(_tag("Alignment"))
                   if el.get("name") == sidecar["source"]["alignment_name"]]
        if len(matches) != 1:
            raise ValueError("sidecar alignment_name must select exactly one LandXML alignment")
        aln_el = matches[0]
        units = root.find(f'{_tag("Units")}/{_tag("Metric")}')
        if units is None or units.get("linearUnit") != "meter":
            raise ValueError("sidecar import requires LandXML metric metre units")
        if float(aln_el.get("staStart", "0")) != 0 or any(
            el.tag in {_tag("StaEquations"), _tag("StaEquation")} for el in aln_el.iter()
        ):
            raise ValueError("sidecar import does not support station equations or nonzero chainage origins")
    if aln_el is None:
    ...
    if sidecar is not None:
        validate_sidecar(sidecar, aln, meta)
    text = emit_toml(aln, meta, sidecar)
    if sidecar is not None:
        report = validate(tomllib.loads(text), sidecar["known_station_ids"], {meta.line_id})
        if not report.ok:
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    ap.add_argument("--sidecar", type=Path, help="Revision-bound station/civil/cant TOML companion.")
    ap.add_argument("--design", type=Path, help="Design whose line/station IDs the sidecar references.")
    ap.add_argument(
        "--output",
        type=Path,
    ...
    try:
        text = convert(args.input, meta, platform_length_default_m=args.platform_length_default,
                       sidecar_path=args.sidecar, design_path=args.design)
