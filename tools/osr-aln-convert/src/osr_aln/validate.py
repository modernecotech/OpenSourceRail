...
PRESETS: dict[str, PresetLimits] = {
    "heritage-tram": PresetLimits(
        name="heritage-tram",
        min_curve_radius_m=25.0,
        max_gradient_per_mille=80.0,
        max_cant_mm=40.0,
        compatible_consists=frozenset({"tram-2car"}),
    ),
    "standard-urban": PresetLimits(
        name="standard-urban",
        min_curve_radius_m=200.0,
        max_gradient_per_mille=40.0,
        max_cant_mm=150.0,
        compatible_consists=frozenset(
            {"light-metro-3car", "metro-4car"}
        ),
    ),
    "standard-metro": PresetLimits(
        name="standard-metro",
        min_curve_radius_m=300.0,
...
def validate_file(
    ...
    with path.open("rb") as f:
        doc = tomllib.load(f)
    return validate(doc, known_station_ids=known_station_ids)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    report = validate_file(args.aln, known_station_ids=known_ids)
