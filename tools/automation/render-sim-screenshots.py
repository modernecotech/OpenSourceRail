...
def default_out_dir(scenario: Path) -> Path:
    return scenario.resolve().parent / "engineering/screenshots"
...
def capture_visualizer(scenario: Path, out_dir: Path, slug: str) -> Path:
    html_path = BUILD_DIR / f"{slug}-network-visualizer.html"
    png_path = out_dir / f"{slug}-network-visualizer.png"
    run(
        [
            "cargo",
            "run",
...
def run_sim_trace(scenario: Path, duration_s: int, slug: str) -> tuple[Path, Path]:
    csv_path = BUILD_DIR / f"{slug}-sim.csv"
    json_path = BUILD_DIR / f"{slug}-sim.json"
    run(
        [
            "cargo",
            "run",
...
def plot_dashboard(
    ...
    png_path = out_dir / f"{slug}-simulation-dashboard.png"
    ...
    title = (
        f"{city_name} simulation trace: motion, dwell, charging, roof PV, "
        "and battery state"
    )
    subtitle = (
    ...
    fig.text(
        0.04,
        0.035,
        "Generated by tools/automation/render-sim-screenshots.py from osr-sim CSV output.",
        fontsize=9,
        color="#6b7280",
    )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="output folder (default: <scenario-folder>/engineering/screenshots)",
    )
    parser.add_argument("--duration", type=int, default=7200)
    args = parser.parse_args()

    args.out_dir = args.out_dir or default_out_dir(args.scenario)
    args.out_dir.mkdir(parents=True, exist_ok=True)
    BUILD_DIR.mkdir(parents=True, exist_ok=True)

    with args.scenario.open("rb") as handle:
        scenario_doc = tomllib.load(handle)
    slug = args.scenario.stem.lower()
    city_name = str(scenario_doc.get("scenario", {}).get("name", slug.title()))

    visualizer = capture_visualizer(args.scenario, args.out_dir, slug)
    csv_path, json_path = run_sim_trace(args.scenario, args.duration, slug)
    dashboard = plot_dashboard(
        csv_path, json_path, args.out_dir, slug, city_name
    )
