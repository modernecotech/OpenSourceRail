...
def run(city, rounds, workers, operators, output):
    ...
    with tempfile.TemporaryDirectory(prefix='osr-supervision-load-') as directory, ExitStack() as cleanup:
        ...
        servers=[]
        class LoadServer(ThreadingHTTPServer):
            request_queue_size = 128
        for handler in (Handler, FuxaReadHandler):
            server=LoadServer(('127.0.0.1',0),handler);server.store=store;server.config=config
            ...
            for cycle in range(rounds):
                ...
                for index,asset in enumerate(package['equipment']):
                    ...
                    for name,m in asset['measurements'].items():
                        ...
                        def publish(data=data):
                            data={**data,'source_timestamp':datetime.now(timezone.utc).isoformat()}
                            return request(public,'/telemetry',data,'controller-'+data['source_id'])
        ...
        report=dict(schema='osr-supervision-load/1',city=city,
            ...
            rounds=rounds,workers=workers,operators=operators,cycle_seconds=cycles,request_timings=timings,
...
def main():
    ...
    if not 1<=args.rounds<=100 or not 1<=args.workers<=64 or not 1<=args.operators<=32:
        parser.error('rounds 1..100, workers 1..64, operators 1..32')
    result=run(args.city,args.rounds,args.workers,args.operators,args.output)
