...
def render_report(
    ...
    out.extend([
        ...
        "## Review Artifacts",
        "",
        f"- Operations bundle: [`{bundle_path.name}`]({bundle_path.name})",
        f"- Evidence matrix CSV: [`{matrix_path.name}`]({matrix_path.name})",
        f"- Manufacturing schedule CSV: [`{slug}-manufacturing-schedule.csv`]({slug}-manufacturing-schedule.csv)",
        f"- Manufacturing materials CSV: [`{slug}-manufacturing-materials.csv`]({slug}-manufacturing-materials.csv)",
        f"- Manufacturing verification CSV: [`{slug}-manufacturing-verification.csv`]({slug}-manufacturing-verification.csv)",
        f"- QA register CSV: [`{slug}-qa-register.csv`]({slug}-qa-register.csv)",
        f"- Maintenance schedule CSV: [`{slug}-maintenance-schedule.csv`]({slug}-maintenance-schedule.csv)",
        "",
        "## Accreditation Use",
        "",
