...
| [`operating-readiness.py`](operating-readiness.py) | Compile every tracked city ERP/component/supervision profile against its own asset and twin evidence, report exact on-demand and operator-owned inputs, and detect report drift |
...
| [`buildable-trainset.sh`](buildable-trainset.sh) | Generate the LM3 product tree, supplier/COTS registers, 29 factory drawing briefs covering all 62 locally made rows, package readiness, first-article work packages and evidence status |
...
| [`platform-recovery.py`](platform-recovery.py) | Capture private coordinated platform checkpoints and rehearse fresh-volume recovery; see the [recovery guide](../../docs/operating/platform-recovery.md) |
