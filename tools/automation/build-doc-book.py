...
def _doc_sources() -> list[SourceDoc]:
    ...
    engineering_docs = _markdown_under("engineering")
    engineering_docs.extend(_markdown_under("control-electronics"))
    add("Engineering, Control Electronics and Formal Assurance", sorted(engineering_docs))

    mechanical_docs = [
        REPO_ROOT / "design/README.md",
    ...
    for relative in (
        ...
        "design/component-catalogue/catalog/modular-fiberglass-body",
    ):
    ...
    country_briefs = [
        path
        for path in sorted((REPO_ROOT / "cities/catalogue").glob("*/*/NATIONAL-BRIEF.md"))
...
def _validate_doc_sources(docs: list[SourceDoc]) -> None:
    ...
    excluded = {"docs/README.md", "docs/INDEX.md", "cities/catalogue/README.md"}
...
def _city_models() -> list[CityModel]:
    ...
    for design_path in sorted((REPO_ROOT / "cities/catalogue").glob("*/*/*/design.toml")):
        ...
        rel_parts = city_dir.relative_to(REPO_ROOT / "cities/catalogue").parts
...
def _city_directory_flowables(
    ...
    return [
        ...
        Paragraph(
            "City models are grouped as cities/catalogue/<region>/<country>/<City>/. "
