...
| Operating voltage | Train pack voltage, approximately 650–700 V nominal and 740 V normal maximum |
| Contact / insulation rating | At least 1 000 V DC |
...
| Onboard power electronics | Per-car HV contactor/protection/IMD charge rack; the station-side DC/DC regulates dock current and the roof PV uses its own MPPT stage |
...
| Voltage | Follows the train pack: approximately 650–700 V nominal, 740 V normal maximum; hardware rated at least 1 000 V DC |
| Max current | 825 A equipment limit; approximately 714–769 A at the 500 kW operating point |
| Ramp rate | Controlled by the station DC/DC under train BMS limits relayed by `osr-aux-power` |
...
| Onboard sink | Traction battery through per-car HV contactor, fuse, IMD/HVIL, and BMS-controlled charge rack; no onboard AC conversion |
...
|---|---|
| Modules per car | 12 |
| Mount styles | 6 bonded flexible laminates; 6 raised rigid panels on rails and edge clamps |
| String hardware | Module junction boxes, two string raceways, MPPT combiner, fire-isolation switches |
| Battery interface | PV downlink cable gland into the per-car MPPT/DC charge rack |
...
|---|---|
| `standard-urban` geometry | Yes — 49.5 m consist, 90 m min curve OK, 50 ‰ max grade OK |
...
| `standard` station | Yes — 59.5 m derived platform for the 49.5 m consist |
| `major` station | Yes |
