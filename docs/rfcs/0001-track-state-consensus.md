...
## 10. Interoperability and Migration

Operators migrating from legacy systems need a path that doesn't require flag-day cutover.

- **Shadow mode.** OSR consensus runs alongside the legacy ZC, consuming the same inputs, producing MAs that are compared but not enforced. When the shadow matches reality for a defined period, operation can cut over.
- **Legacy inputs.** W-Nodes can ingest from existing track circuits and axle counters as additional `TrainPositionReport` sources (with explicit `source_mask` bits identifying legacy origin). This lets a region run OSR consensus atop unmodified detection hardware while rolling stock and detection are gradually upgraded.
- **Legacy outputs.** A dedicated "OCS gateway" W-Node can translate MAs into legacy continuous cab-signal codes for older rolling stock still running lineside signal protection. This is a transitional feature; it is not a supported long-term mode.

Interoperability modules live in `crates/osr-legacy-*` and are explicitly out of the SIL-4 boundary; they are advisory only.

## 11. Formal Verification Plan

Four layers of verification, each proving different properties:

### 11.1 Protocol layer (TLA+)
...
### 11.3 Rust-code layer (Creusot where feasible, tests elsewhere)
