...
OpenSourceRail commits to **three track-geometry presets**,
...
> bridge. The three presets above assume above-ground operation
...
## 3. Why three presets, not a continuum
...
Three presets cover the actual range of urban-rail deployments in
the target regions:
...
All three presets respect **UIC 505-1 structure gauge**
...
```text
  ...
  ──────────────   ───────────────────     ─────────────────────────
  ≤ 300 k          tram-2car               standard-urban
...
| **v3** ✅ | Parametric track components at [`design/component-catalogue/src/osr_mech/track/`](../../design/component-catalogue/src/osr_mech/track/) (done 2026-04-22), including the controlled 1:9 turnout kit. **Civil-tool bridge:** [`tools/osr-aln-convert/`](../../tools/osr-aln-convert/) — stdlib-only Python toolchain: `landxml-to-osr-aln` reads LandXML 1.2 exports from Civil 3D / Bentley OpenRail / Trimble BC / QGIS; `current-network-to-osr-aln` deterministically exports the generated three-line Samawah planning package with source hashes; `osr-aln-validate` enforces the 8 hard gates + 3 soft gates and resolves both line and station IDs against a deployment's design.toml. 26 passing tool tests. Remaining for v3.1: Trimble TCL CSV reader, Bentley cant-extension parser, and CERN-OHL-S v2 relicensing. | v1 |
