# RFC 0015 — Driverless operation (GoA 4) as the sole train profile
...
Train Operation, UTO) system from day one**. The rolling stock has
no driver cab and the operations model has no driver role.

This RFC sets the safety, hardware, and operations envelope for
that commitment — specifically, the onboard obstacle-detection
...
1. **Lower total cost.** A cab costs ~$270k per trainset when
   wiring, cab displays, HVAC, controls, and emergency exits are summed.
...
2. **Simpler trainset.** Removing the cab takes out roughly
   40 kg of wiring, four cab HMIs (status, mirror monitors, rear-view,
...
3. **Simpler operations.** Safety responsibilities are assigned directly
   to OCC, station staff, maintenance, and the onboard automation stack;
   the repository carries no train-driver procedure family.
...
- Removal of cab-only software and interfaces.
...
- Cab-retained variants. They are outside the project scope.
...
| Cab touchscreen | No driver | Remote-assist web console (OCC) |
...
| Train-driver procedures | No driver | Dispatcher S1–S7 + Station-staff T1–T5 absorb |
| Dead-man controller | No driver to monitor | — |
| Driver display software | No driver to display to | Passenger PIS already exists |
...
Earlier station concepts made PSDs optional for `halt` archetypes.
Under GoA 4, **PSDs are mandatory at every station
...
The driverless trainset makes its safety case through:
...
- **No fatigue, no distraction, no panic.** Driverless trains
  ...
  operations (including DLR, Copenhagen, and Singapore NSL).
...
| Goal | Claim | Evidence |
|---|---|---|
| **G15** | `osr-obstacle-detect` produces a restrictive verdict when any sensor reports an obstacle inside the current stopping-distance envelope; confident light-debris classifications may downgrade to `CrawlOnly`, all other in-envelope detections produce `EmergencyBrake` | O1 Kani harness + proptest |
...
| **G19** | Passenger-intercom-triggered stop is delivered within 3 s of press | `osr-passenger-assist` state-machine tests + `osr-sim` intercom-to-brake integration test (one simulation tick) |

### 6.3 Residual risks explicit

Driverless operation carries residual risks that must be handled without
an onboard operator. Each one is explicitly acknowledged:
...
- **Passenger medical emergency mid-journey**: intercom to OCC;
  OCC dispatches medical assistance to the next station.
...
### 7.1 Unattended roles only

RFC 0013 assigns all operational decisions to the onboard automation,
OCC dispatcher, station staff, maintenance, or control-centre roles. It
contains no onboard train-driver role or mixed-fleet procedure set.
...
- Front wall: just a nose cone with the §5.1 sensor cowl + a
  coupler interface.
- No windscreen, no cab display, no master controller, no cab door,
...
- A wired pendant with: forward / reverse select, 0–15 km/h
  movement request, emergency stop.
- The keyswitch enables a slow-speed (≤ 15 km/h) protected shunt
  under ATP supervision — used only to position a stopped trainset
  for rescue coupling inside an isolated work zone.
...
| Cab controls + displays + wiring | ~150 | ~$87 000 |
| Cab HVAC | ~120 | ~$16 000 |
...
### 10.2 Exclusive unattended train image

[`osr-trainset-image`](../../crates/osr-trainset-image/) is the single
onboard integration entry point. It re-exports the GoA 4 stack (ATP,
ATO, brake, derailment, door control, fire safety, obstacle detection,
odometry, passenger assistance, and TCMS). There is no cab feature flag,
driver-display crate, or dead-man controller in the workspace.
...
- **RFC 0013 (operations rulebook)** — OCC and station staff own the
