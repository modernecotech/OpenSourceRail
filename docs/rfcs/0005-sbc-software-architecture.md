...
**MVB and WTB are not in the map** — deprecated per
[ARCHITECTURE.md §D5](../ARCHITECTURE.md#d5-rolling-stock) in favor of
TCN-E. Crates that would have spoken MVB in a legacy vehicle instead
publish and subscribe on TCN-E topics.
...
| Crate | Class | SIL | Scope |
|---|---|---|---|
| **osr-traction** | T-ECU/S | 4 | VVVF inverter firmware. Field-oriented control of the induction or PMSM motor; torque command from `osr-ato`; anti-slip; anti-slide. Targets the open SiC reference inverter in [RFC 0003 §4.3](0003-samawah-reference-deployment.md). |
| **osr-bms** | T-ECU/S | 4 | Battery Management System for Na-ion and LFP packs. Cell balancing, SoC/SoH estimation, thermal limits, contactor control, cell-out derating. Exposes pack state to `osr-ato` for energy-optimal driving. |
...
| **osr-aux-power** | T-ECU/A | 2 | Auxiliary converter control — 24 V DC, 110 V DC, 400 V AC for HVAC, lighting, compressors. Load-shed logic under low SoC. |
...
| **osr-energy-site** | W-SBC | 2 | Per-site PV + Na-ion + grid-tie controller per [RFC 0002](0002-energy-sizing.md). Already scoped architecturally; listed here so it has a slot in the workspace. |
...
|---|---|---|---|---|---|
| O1 | `osr.train.ma` | `osr-atp` (mirror of `osr-interlocking` output) | `osr-ato`, `osr-dmi`, `osr-event-recorder` | On change, ≤ 1 Hz | ~200 B |
...
| O4 | `osr.train.brake_apply` | `osr-atp`, `osr-vigilance`, `osr-fire-safety`, `osr-derailment` | `osr-brake` | On event | ~20 B |
...
`osr-bms`, `osr-door-control`, `osr-vigilance`, `osr-fire-safety`,
`osr-derailment`), and one or more T-ECU/A units (running `osr-ato`,
`osr-tcms`, `osr-dmi`, `osr-pis-onboard`, `osr-hvac`, `osr-lighting`,
...
## 10. Legacy practice this RFC deprecates

Consistent with [ARCHITECTURE.md](../ARCHITECTURE.md#2-design-principles)
principle 3 ("deprecate, don't reimplement"), this RFC explicitly
deprecates:

- **MVB and WTB (IEC 61375-1).** Replaced by TCN-E (IEC 61375-2-3) over
  TSN. No crate in §4 speaks MVB; no retrofit shim is planned.
- **Per-vendor ECU stacks.** Legacy trains carry 20–50 ECUs from a
  handful of Tier-1s. OpenSourceRail vehicles run the §4 crates on
  two hardware SKUs (T-ECU/S and T-ECU/A).
- **Track circuits as primary train detection.** Onboard `osr-odometry`
  + wayside `osr-balise` + `osr-consensus` replaces them; track circuits
  retained only as a secondary sensor on legacy retrofits, not new-build.
- **Dedicated DMI vendors.** `osr-dmi` on commodity touchscreens over
  standard HDMI + USB.
- **Proprietary CBM portals.** `osr-cbm-backend` consumes OTLP from every
  ECU; any maintainer with a browser uses the same portal for every
  fleet.
- **Closed-loop smart card AFC.** `osr-afc` is account-based and
  mobile-money-first per §D4.

Each deprecation carries a short rationale in the §4 scope column
("why this, not legacy X") and is traceable to the decision in
ARCHITECTURE.md.
...
4. **`osr-vigilance`** — trivial; folded into `osr-atp`'s RFC or a
   short standalone follow-up.

### Phase 2b — Onboard traction partition

5. **`osr-bms`** — Na-ion/LFP pack management.
...
9. **`osr-tcms`**, **`osr-dmi`**, **`osr-event-recorder`**.
