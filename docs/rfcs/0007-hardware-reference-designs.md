...
| **v2** 🔶 (spec only) | T-ECU/S baseboard schematic **specification** at [`hardware/t-ecu-s/schematics/v2-spec/`](../../hardware/t-ecu-s/schematics/v2-spec/) — block diagram, power budget, RP2350 + CM5 pinouts, 33-connector table, safety-net routing rules (done 2026-04-22). KiCad capture + gerbers + BOM are v2.1 per-vendor deliverables; each stays under CERN-OHL-S v2. Remaining classes (T-ECU/A, W-SBC, S-SBC) gated on v2.1 of T-ECU/S. | v1 |
