...
| **v2** | Integration with `osr-interlocking` — `SectionIntrusion` consensus entry, `section_available_to` consults intrusion state | v1, RFC 0001 |
| **v3** | Sim integration: wayside sensor faults in `osr-sim` feed intrusion-detect the same way RFC 0015 v2.1 does for onboard | v2 |
| **v4** | Rulebook amendments (S7 + M7) — one-sentence + Why: format per RFC 0013 | v2 |
