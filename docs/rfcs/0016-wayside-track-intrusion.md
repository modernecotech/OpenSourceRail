...
| **v3** ✅ | Sim integration: new `FaultKind::WaysideIntrusion { section, state }` in `osr-sim::fault`; the sim tick emits one `SectionIntrusion` consensus entry per active fault through `MaLogBackend::emit_intrusion` (both `SimulatedLog` and `ConsensusBackend` backends). Demonstrator scenario at [`designs/west-asia/Iraq/Samawah/samawah-wayside-intrusion.toml`](../../designs/west-asia/Iraq/Samawah/samawah-wayside-intrusion.toml) stages Present / Unknown / Present events across three sections during AM peak; run completes with zero invariant violations — the interlocking's gate (d) held MA back without any train entering the flagged sections. (done 2026-04-23) | v2 |
