...
cost ≈ $7 600 per section — small fraction of the $3.0 M per km
civil cost.
...
| **v2** ✅ | Integration with `osr-interlocking`: `EntryPayload::SectionIntrusion` and `IntrusionState` (`Clear`/`Unknown`/`Present`) feed gate (d); only `Clear` permits movement, while `Present`, `Unknown`, or a missing verdict withhold MA. | v1, RFC 0001 |
| **v3** ✅ | Sim integration: `FaultKind::WaysideIntrusion { section, state }` emits committed consensus entries. A demonstrator scenario stages Present / Unknown / Present events across three sections without any train entering a flagged section. | v2 |
