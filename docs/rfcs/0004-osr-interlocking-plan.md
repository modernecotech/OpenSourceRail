...
- `ConsensusBackend` caches a `DerivedState` from the committed prefix;
  gate checks stay O(1) in log length.
- `ConsensusBackend::propose` now calls `cluster.run_until_committed`
  ...
  proves the consensus-backed simulator produces clean MA summaries.
