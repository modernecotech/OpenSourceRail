...
1:18.5) across every deployment, plus a separate tram-geometry
1:5 for the `heritage-tram` preset. This RFC fixes the physical
...
|---|---|---|---|
| **1:5** | 15 km/h | `heritage-tram` only | Tight street turnouts, tram geometry |
...
Switch geometry is where railway engineering has the most
legacy variation — every national network historically standardised
on its own tangents (British 1:8, French 1:9.25, Russian 1:11,
etc.). A new-build project gets to pick once. Picking three
covers the full operational envelope:
...
- **1:5** handles heritage-tram street running — a different
  geometry family that never mixes with the above.
...
| Aspect | Choice |
|---|---|
| **Rail type** | UIC60E2 (60 kg/m) for 1:9 / 1:14 / 1:18.5; Ri60 for 1:5. Matches the parent rail profile from RFC 0009. |
| **Switch blade** | Flexible switch blade, continuous machined profile; no bolt-on tip. 12 m long for 1:9, 20 m for 1:14, 26 m for 1:18.5. |
...
Tramway street crossings, where the tram crosses another tram
line at grade in a city street, are the only upstream-supported
diamond — handled in the `heritage-tram` preset only, as a
special case.
...
| Tangent | Angle | Switch lead length | Through-radius | Divergent speed |
|---|---|---|---|---|
| 1:5 | 11.3° | 15 m | 50 m | 15 km/h |
| 1:9 | 6.3° | 23 m | 190 m | 40 km/h |
| 1:14 | 4.1° | 37 m | 500 m | 60 km/h |
| 1:18.5 | 3.1° | 49 m | 900 m | 80 km/h |
