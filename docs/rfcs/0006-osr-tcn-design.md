...
- **Protocol bridging.** Deployment-specific adapters are outside TCN-E.
...
| **App** (A) | 3 | Best-effort, preemptible | `hvac.status`, `pis.display`, `event.record` |
...
| **v2** ✅ | `osr-sim` publishes the odometry snapshot and consumes it at the ATP boundary through `MockTcn`; transport failure becomes unknown position and trips fail-restrictively | v1 |
...
2. **Payload schema evolution.** Today a new topic requires a new schema and
   all consist ECUs must agree on the topic-map hash before entering service.
...
- [x] Expand the landed v1 crate from 4 to at least 6
      proptest-verified transport/registry properties.
...
The v1, v1.5, and v2 implementations have landed. The cross-crate payload
criterion remains open: the adapter currently owns the `TcnPayload` wrapper so
SIL-4 evaluator crates do not acquire a runtime dependency on the SIL-2 bus,
as required by RFC 0005 §13.
