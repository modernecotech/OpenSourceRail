...
- **Line 1** — 25.6 km / 14-station southwest-north radial.
- **Line 2** — 12.8 km / 8-station southeast-northwest radial with the
  generated main depot-terminal.
- **Line 3** — 12.0 km / 8-station west/east connector.

Total generated network: **~50 km of double-track light metro, 21 unique stations, and 108 trainsets: 97 peak, no dedicated depot-service rotation, 8 planned spares, and 3 cold reserves**.
...
|---|---:|---:|---:|---|---|
| line-1 | 25.6 km | 9 | 53 | N Mid ↔ SW Outer | Long southwest-north radial through the strongest generated anchor chain |
| line-2 | 12.8 km | 6 | 28 | SE Mid ↔ N Mid | Southeast radial, including the generated depot-terminal / main maintenance anchor |
| line-3 | 12.0 km | 6 | 27 | E Mid ↔ W Inner | Shorter connector filling the west/east catchment |
| **Total** | **50.4 km** | **21 unique** | **108** | | |

### 3.2 Transfers And Interchange Caveat

The generated README reports **1 interchange-class station** and **100%
multi-line transfer reachability**, meaning every line-pair shares at
least one generated station ID in the current graph. In plain terms: the
current output now has a single station-complex transfer pattern, while
the exact surveyed platform arrangement still needs local alignment
review.
...
| Fleet (revenue) | 97 × 3-car trainsets |
...
- **Transfer stitching still needs local design.** The current output has
  one interchange-class station and 100% line-pair station sharing. A local
  planning pass must decide whether that becomes a true shared-platform
  transfer, adjacent paid-area transfer, or a separately phased stop.
...
- **50 km / 21 stations / 108 trainsets** is a full multi-phase build-out
...
- [RFC 0029](0029-maintenance-schedule-system.md) creates the baseline
  asset schedule for 108 trainsets, 21 stations, 50.4 route-km, energy
...
| Component | kWh/day |
|---|---|
| Scheduled traction service (23,436 train-km/day × 3 cars × 4 kWh/car-km) | ≈ 281,000 |
| Depot/deadhead allowance (+8%) | ≈ 22,000 |
| **Generated traction-energy model** | **≈ 304,000 kWh/day = 304 MWh/day** |
...
needs roughly **110.9 GWh/year** of traction energy before station/building
auxiliaries.
The current generated timetable schedules **1,395 one-way train journeys
per day** across the three lines, equal to **23,436 train-km/day** before
...
|---|---:|
| On-site station/depot PV | 13.7 MW / 30.0 GWh/yr |
| Traction demand | 110.9 GWh/yr |
| Shortfall before supplemental plant | 81.0 GWh/yr |
| Dedicated solar plant, incl. 115% coverage margin | 42.5 MW / 93.0 GWh/yr |
...
- Station/depot stationary storage: **54.5 MWh LFP**
  distributed across 30 energy sites.
- Main depot-class site: **40 MWh**.
- Standard/major/interchange/terminal sites: **500 kWh** each by
  template tier.

Onboard: **108 trainsets × 675 kWh = 72.9 MWh gross** fleet battery
capacity (**58.3 MWh usable**), of which 97 trainsets are peak-revenue fleet
and 11 are planned spares/cold reserve.
...
Powered stations and the single main-heavy depot are represented as charging
locations. During normal turnback the train uses the shared **500 kW
passenger-platform cabinet**; during a service hold it can use a **150 kW
low-C service charger**, topping toward 95% SoC where the timetable permits.
Healthy trains stable overnight at powered passenger stations; red defects,
failed isolation, failed security, or heavy-maintenance-due sets return to
the main-heavy depot.

One powered service point on each line is the designated turnaround-service
point. During
...
generated all-in planning CAPEX is **$461 M** with all 108 sets included.
...
Applying the current cost templates to the generated 50.4 km city
instance:

| Item | Cost (USD) |
|---|---|
| Dedicated 42.5 MW solar plant + interconnection | $34 M |
| Station/depot charging microgrids | $3.3 M |
...
| **Visible energy adders in generated CAPEX** | **≈ $37 M** |

The dedicated plant adds about **$34 M** to infrastructure CAPEX and
about **$0.51 M/year** of plant O&M, but removes the earlier
...
25 kV AC OCS plus traction substations; scaling that to 50.4 km would be
on the order of **$300 M** before local feeder works. The OSR energy
...
### D5. Rolling Stock
- 97 peak-revenue + 11 spare/cold-reserve 3-car trainsets, 324 cars total;
...
- **Generated depot set:** 1 main-heavy depot-terminal. The main depot
  carries the heavy workshop, CBM, training wing, and 40 MWh storage
  role; healthy trainsets stable overnight at powered passenger stations
  rather than at a depot parking yard.
...
108-trainset timetable, applies station/depot charging and roof PV, enforces
...
4. **Fleet and charging validation.** Stress the 97+11 trainset fleet,
