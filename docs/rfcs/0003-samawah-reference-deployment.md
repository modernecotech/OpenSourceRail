...
- **Line 2** — 21.9 km / 11-station southeast-northwest radial with the
  generated main depot-terminal.
- **Line 3** — 11.0 km / 8-station west/southeast connector.

Total generated network: **~59 km of double-track light metro, 32 unique stations, and 149 trainsets: 134 peak, no dedicated depot-service rotation, 12 planned spares, and 3 cold reserves**.
...
|---|---:|---:|---:|---|---|
| line-1 | 25.6 km | 13 | 64 | SW Outer ↔ N Mid | Long southwest-north radial through the strongest generated anchor chain |
| line-2 | 21.9 km | 11 | 57 | SE Mid ↔ NW Outer | Southeast-northwest radial, including the generated depot-terminal / main maintenance anchor |
| line-3 | 11.1 km | 8 | 28 | W Inner ↔ SE Mid | Shorter connector filling the west/southeast catchment |
| **Total** | **58.5 km** | **32 unique** | **149** | | |

### 3.2 Transfers And Interchange Caveat

The generated README reports **2 interchange-class stations** because
...
**73.3–117.3 M annual paid trips**, based on 50–80% use of practical
...
| Metric | Value |
|---|---|
| Route-km (double track) | ~50.4 km |
| Stations (unique) | 31 |
| Lines | 3 (auto-planned by `osr-design`) |
| Interchange-class stations | 1 shared multi-line interchange complex |
| Multi-line transfer reachability | 100% of line-pairs sharing a station ID in the current generated graph |
| Fleet (revenue) | 115 × 3-car trainsets |
| Fleet (dedicated depot-service rotation) | 0 — off-peak uses surplus peak fleet |
| Fleet (spare + cold-reserve) | 12 × 3-car trainsets |
| Average inter-station spacing | Generated from [`SpacingConfig`](../../crates/osr-routing/src/station.rs): 1.2 km central, 2 km urban, 3 km suburban, and up to 5 km in the lowest-demand outer fringe |
...
- **59 km / 32 stations / 149 trainsets** is a full multi-phase build-out
...
|---|---|---|---|
| AM peak (07:00–09:00) | 3 min | 3 min | 3 min |
| Midday (09:00–15:00) | 6 min | 6 min | 6 min |
| PM peak (15:00–17:00) | 3 min | 3 min | 3 min |
| Evening (17:00–23:30) | 6 min | 6 min | 6 min |
...
- 3-car, 49.5 m consist with low-floor centre door zones (light metro class)
- LFP onboard battery, 540 kWh usable / 675 kWh gross per trainset (180 / 225 kWh per car), sized for roughly one route length with station opportunity charging; the current 25.6 km longest line uses ~307 kWh one-way at 12 kWh/train-km, leaving ~233 kWh of usable margin above the protected reserve
- Six heavy-vehicle PMSM/controller channels with Rust supervisory control
...
- [RFC 0029](0029-maintenance-schedule-system.md) creates the baseline
  asset schedule for 149 trainsets, 32 stations, 58.5 route-km, energy
...
| Component | kWh/day |
|---|---|
| Scheduled traction service (27,204 train-km/day × 3 cars × 4 kWh/car-km) | ≈ 326,000 |
| Depot/deadhead allowance (+8%) | ≈ 26,000 |
| **Generated traction-energy model** | **≈ 352,000 kWh/day = 352 MWh/day** |
...
needs roughly **129 GWh/year** of traction energy before station/building
auxiliaries.
The current generated timetable schedules **1,395 one-way train journeys
per day** across the three lines, equal to **27,204 train-km/day** before
the depot/deadhead factor.

### 5.2 PV generation

The generated city model uses a **6.0 peak-sun-hour**
...
each 3-car trainset carries **15.12 kW nameplate** at **65% usable**,
...
|---|---:|
| On-site station/depot PV | 12.35 MW / 27.0 GWh/yr |
| Traction demand | 128.7 GWh/yr |
| Shortfall before supplemental plant | 101.7 GWh/yr |
| Dedicated solar plant, incl. 115% coverage margin | 53.4 MW / 116.9 GWh/yr |
...
- Station/depot stationary storage: **52.25 MWh LFP**
  distributed across 29 energy sites.
- Main depot-class site: **40 MWh**.
- Standard/major/interchange/terminal sites: **500 kWh** each by
  template tier.

Onboard: **149 trainsets × 675 kWh = 100.6 MWh gross** fleet battery
capacity (**80.5 MWh usable**), of which 134 trainsets are peak-revenue fleet
and 15 are planned spares/cold reserve.
This keeps the current longest-line run above dispatch reserve, but it
does not make the line independent of station charging for repeated
service; terminal and intermediate charging remain part of the operating
assumption.

All six terminal depots/layups are represented as charging locations. During
normal turnback the train uses the shared **500 kW passenger-platform cabinet**; while
held between duties it transfers to a **150 kW low-C depot charger**, topping
toward 95% SoC. The five remote `layup-minimal` sites each carry their RFC
0014-sized 90 kWp PV / 150 kWh buffer; the main-heavy depot retains its 5 MWp
PV / 40 MWh buffer.

One depot on each line is the designated turnaround-service point. During
the 3-minute morning and afternoon peaks, trains make the normal quick
turnaround and are allowed greater battery depletion while the 20% reserve
gate stays active. In the 6- and 12-minute off-peak windows they receive a
**12-minute concurrent clean, safety-release inspection, fault-log download,
and recharge**. The peak fleet's off-peak surplus covers this work, so it
requires **zero additional trainsets and zero incremental fleet CAPEX**. The
generated all-in planning CAPEX is **$664 M** with all 149 sets included.
...
2. **Battery conditioning.** The selected LFP cells prefer operation below 45 °C. Trackside battery enclosures need insulation + passive ventilation minimum; active cooling for high-C discharge periods is justified for the main depot. Onboard packs sit under the seats in thermally managed, fire-resistant compartments with outward venting and localized water mist.
...
Applying the current cost templates to the generated 58.5 km city
instance:

| Item | Cost (USD) |
|---|---|
| Dedicated 53.4 MW solar plant + interconnection | $43 M |
| Station/depot charging microgrids | $3.2 M |
...
| **Visible energy adders in generated CAPEX** | **≈ $46 M** |

The dedicated plant adds about **$43 M** to infrastructure CAPEX and
about **$0.64 M/year** of plant O&M, but removes the earlier
...
25 kV AC OCS plus traction substations; scaling that to 58.5 km would be
...
### D5. Rolling Stock
- 134 peak-revenue + 15 spare/cold-reserve 3-car trainsets, 447 cars total;
  depot service uses off-peak peak-fleet surplus.
- Reference T-ECU hardware across all non-traction-power functions.
- Onboard battery at 540 kWh usable / 675 kWh gross per trainset, LFP.
- Roof PV at 15.12 kW/trainset with low-pressure air-cleaner support in
  dusty service.
...
149-trainset timetable, applies station/depot charging and roof PV, enforces
the clean/inspection service slot, and can
...
4. **Fleet and charging validation.** Stress the 134+15 trainset fleet,
   540 kWh usable / 675 kWh nameplate packs, station chargers, roof PV,
   and air-cleaner parasitic
...
2. **Demand reality check.** The generated finance model now uses the system's practical capacity as the revenue basis: 73.3–117.3 M annual paid trips at 50–80% capacity use. A proper local demand study would refine that capacity-use bracket and its line-by-line distribution. Fleet capacity now has margin; the generated energy plan closes the on-site PV gap with a dedicated solar plant, but its land/PPA counterpart, interconnection, outage reserve, and export-settlement terms need local feasibility work.
...
1. **Replace planning OSR-ALN with surveyed controls.** The current three-line
   package is reproducibly exported and validated, but its GIS horizontal
   trace, zero-datum vertical placeholder, and absent cant must be replaced by
   signed survey/CAD deliverables before construction release.
