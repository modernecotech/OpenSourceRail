...
This RFC proposes a concrete reference deployment for OpenSourceRail: a **three-line auto-planned urban rail network** serving **Samawah (As-Samawah), capital of Al-Muthanna Governorate, Iraq**. The current source of truth is [`designs/west-asia/Iraq/Samawah/design.toml`](../../designs/west-asia/Iraq/Samawah/design.toml), emitted by `osr-design` from OpenStreetMap anchors plus the WorldPop residential-population demand surface. Every inter-station polyline is generated by the routing pipeline rather than sketched "as the crow flies". The current generated network is:
