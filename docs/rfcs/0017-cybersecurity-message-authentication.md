...
The `osr-secbus` v1 crate does not perform this freshness check: it
authenticates opaque bytes against a deployment key registry before
deserialisation. The consensus wire integration in rollout v2 is
responsible for deserialising verified entries, checking monotonic
`entry_id`, and rejecting entries whose `timestamp_ns` is more than
**60 seconds** older than the cluster's current leader clock.
...
| **v2** ✅ | Authenticated consensus application ingress signs entry id, timestamp, category, and track-entry bytes as one body; the leader rejects unknown, forged, malformed, stale, future, replayed, and metadata-mismatched proposals before append; committed consumers independently verify the stored envelope and signed category before deserialising the track entry. (done 2026-08-11) | v1, RFC 0001 |
| **v3** ✅ | `osr-sim::ConsensusBackend` uses the authenticated proposal and committed-consumer paths. End-to-end tests cover forged signature, altered payload, replay, stale/future timestamp, unknown issuer, metadata mismatch, and category downgrade. (done 2026-08-11) | v2 |
