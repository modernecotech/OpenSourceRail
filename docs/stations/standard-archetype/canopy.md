...
> **Status: controlled auxiliary-module structural envelope.** The passenger
> platform baseline remains the repeatable 6 m cantilever kit. The 85 × 22 m
> worked roof below has been decomposed into repeatable 8.5 × 22 m auxiliary
> bays with shared transverse frames by `osr_mech.station.auxiliary_canopy`.
> It remains subject to deployment structural/foundation approval and is not
> a stamped site drawing.
...
| Column spacing along track | 8.5 m (11 shared transverse frames over 10 bays / 85 m) |
...
- Count: 2 rows × 11 frames = **22 columns** for the 10-bay worked row.
...
- Total: 11 shared transverse trusses for 10 adjacent roof bays.
...
| Parameter | Value |
|---|---|
| Factory roof/PV bay | 8.5 × 22 m, 187 m² gross |
| Roof/PV bays per worked canopy | 10 |
| Effective nameplate after 15 % packing allowance | ~318 kWp |
| Cell/module count | Supplier-controlled within the factory-bonded roof envelope |
| Self-consumption (charging trains + station services) | ~20 % |
| Export to grid | ~80 % |

The controlled calculation uses 200 W/m² gross density and a 15 % packing
allowance, matching `station/solar_roof.py`. A deployment energy model applies
temperature, orientation, soiling, inverter, and availability losses separately.
...
total lift. Distributed across 22 columns = 50 kN per column.
...
| Line | Qty | Unit |
|---|---|---|
| HSS 200×200×8 mm column | 84.7 m | 22 × 3.85 m, S355 |
| HSS 150×150×6 mm truss chord | 484 m | two 22 m chords × 11 frames, S355 |
| Truss web/diagonal set | 11 frame sets | final cut lengths from released frame drawing |
| Plate 400×400×20 mm base plate | 22 ea | S355 |
| Plate 450×450×25 mm foundation plate | 22 ea | S355 |
| M30 anchor bolt, 800 mm long | 88 ea | grade 8.8 |
| C25/30 reinforced concrete (foundations) | 57.2 m³ | planning 2.6 m³ × 22; site calculation governs |
| #25 rebar | ~2.6 t | planning scale; site calculation governs |
| Factory-bonded 8.5×22 m solar-roof bay | 10 ea | supplier panel/cell schedule |
| PV combiner/isolation/downlink group | 1 set | maximum ten roof bays per group |
| Gutter/downpipe/lightning/access kit | 10 bay kits | coordinated to site drainage |

Cost remains open until the frame calculation, factory roof supplier, site
foundation design, and electrical string schedule are frozen.
