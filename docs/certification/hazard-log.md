...
| Severity | Critical |
| Frequency without controls | Occasional |
| Controls | RFC 0021 LFP module compartmentation; outward vent; temperature and off-gas detection; charge inhibit; proved string/car-pack isolation; per-car localized water-mist cooling with reservoir/pump/pressure/flow diagnostics; controlled movement to the nearest safe platform unless containment, crash state, traction/braking availability, or fire growth requires immediate emergency braking; IEC 62928-based design and test plan. |
| Residual frequency | Remote |

### H-FI-01A — Battery fire propagates after visible flame reduction

| Field | Value |
|---|---|
| Description | Suppression reduces visible flame but cell heating continues and propagates into an adjacent module. |
| Severity | Critical |
| Frequency without controls | Occasional |
| Controls | Water mist is credited for cooling/propagation delay, not guaranteed cell extinguishment; thermal barriers, outward venting, adjacent-module nozzle coverage, propagation test, continuous post-discharge temperature/off-gas monitoring, evacuation and fire-service handover. |
| Residual frequency | Remote — pending propagation test evidence |

### H-FI-01B — Battery mist unavailable or ineffective

| Field | Value |
|---|---|
| Description | Empty reservoir, failed pump, blocked nozzle, pipe rupture, lost pressure, or absent flow prevents localized cooling. |
| Severity | Critical |
| Frequency without controls | Probable |
| Controls | Independent system per car; reservoir-level, pump-current, pressure and flow monitoring; startup self-test; maintenance proof-flow; fault reported to TCMS/OCC; fire response does not rely on mist to preserve electrical containment or evacuation. |
| Residual frequency | Remote — pending component reliability and proof-flow evidence |

### H-FI-01C — Unsafe stop strategy during onboard fire

| Field | Value |
|---|---|
| Description | Immediate braking strands passengers between stations, or continued movement worsens a fire/crash hazard. |
| Severity | Critical |
| Frequency without controls | Occasional |
| Controls | Staged `osr-fire-safety` outputs distinguish controlled safe-platform stop from emergency braking; independent immediate-danger and safe-movement inputs; OCC alert; route/platform availability check; emergency override for loss of containment, essential control, braking, or safe route. |
| Residual frequency | Remote — pending integrated scenario and evacuation evidence |
