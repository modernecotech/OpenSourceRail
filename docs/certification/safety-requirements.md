...
- **Implementation:** `osr-passenger-assist` + RFC 0015 §5.3. The normal
  intercom action contributes a 500 ppt service-brake request; only an
  authenticated OCC escalation feeds the emergency input.
- **Evidence:** state-machine tests cover immediate latch, link loss,
  authenticated release/escalation, and the `osr-sim` integration test carries
  the request into `osr-brake` within one simulation tick. Release closure
  still requires real media, event-recorder, and incident-workflow evidence in
...
- **Implementation:** `osr-passenger-assist` owns the call lifecycle,
  media-channel request, latched hold, and authenticated OCC
  accept/release/emergency commands; RFC 0013 C2 owns the procedure.
- **Evidence:** state-machine and simulator brake-path tests. The actual
  SIP/audio/video transport, device hardware, and staffed workflow remain
  release gaps.
...
- **Implementation:** All O-series (obstacle-detect), I-series
  ...
  O-odom, B-brake properties carry fail-
  restrictive clauses.
- **Evidence:** 7 Kani harness modules across the SIL-4 crates;
  GSN G1–G24 closure.
...
- **Implementation:** `osr-consensus::authenticated::ProposalVerifier`
  verifies signed proposal metadata, category, freshness, replay state, and
  issuer before proposal and again before committed-entry decode/apply.
- **Evidence:** `osr-secbus` Kani/proptests plus consensus ingress rejection
  and simulator integration tests. Production provisioning and transport/HIL
...
| SR-18 | Remote-assist channel | — | Call/control state machine; media/HW evidence open |
