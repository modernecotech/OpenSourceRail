...
| City Studio | Git-backed city projects, source locks, complete manual line/station/alignment authoring, semantic revision comparison including BCF decisions, editable service for generated/manual routes, atomic all-route day-type headway scenarios and day-plan copying, deterministic manifests, source-locked demand-aware alternatives, controlled GIS/simulation/alignment/IFC4.3 civil jobs, hash-verified GIS/alignment/IFC object/IDS/BCF evidence viewers, searchable multi-asset deterministic BCF topic authoring, evidence-gated coordination management, append-only non-circular revision approvals, and browser/restart persistence acceptance (implemented); native 3D BIM streaming and demand/OD records next |
