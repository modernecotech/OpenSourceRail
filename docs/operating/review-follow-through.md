...
| Proof timeouts and release evidence | [Read-only topology adapters](../../engineering/assurance/formal/topology-adapters.md) separate evaluator logic from map storage. A complete local source-bound run passes 37/41 declared properties and retains every outcome/log; three prior odometry timeouts are closed with unchanged bounds. | Four timeouts remain, including 900-second local follow-up attempts. Local evidence is unattested; exact-candidate CI, consensus refinement and independent acceptance remain open. |
