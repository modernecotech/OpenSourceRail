...
function render() {
  ...
  setCommandRange();
  const overview=[['Equipment',a.name],['OSR source crates',(a.source_crates || []).join(', ') || (a.manufacturing_method?'Method data; no embedded safety controller':'See engineering package')],['OSR identity',a.asset_id],['Engineering revision',a.engineering_revision],['Package status',a.configuration_status || 'active'],['Lifecycle state',a.lifecycle_state],['Physical serial',a.installations.find(i=>!i.removed)?.serial || 'Not installed']];
  if(a.manufacturing_method){const m=a.manufacturing_method,holds=(m.steps || []).filter(step=>step.hold_point).length;overview.push(['Factory method',`${m.method_id} · ${m.document_revision} · ${m.document_status}`],['Work centre',m.work_center],['Planning basis',`${m.crew_size} crew · ${m.planning_cycle_minutes} min · ${m.steps.length} steps / ${holds} hold points`],['Method coverage',`${m.product_ids.length} product rows · ${m.tooling_ids.length} tooling families`]);}
  $('overview').innerHTML=overview.map(([k,v])=>`<span><small>${esc(k)}</small><b>${esc(v)}</b></span>`).join('');
  ...
  if(a.manufacturing_method)links.push(['Manufacturing methods','/design/component-catalogue/catalog/buildable-trainset/manufacturing-methods.md'],['Shop travelers','/design/component-catalogue/catalog/buildable-trainset/travelers/index.md']);
