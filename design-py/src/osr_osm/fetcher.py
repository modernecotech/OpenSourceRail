...
* Overpass rate-limits aggressively (2 slots, ~60s cooldown). The cache is
  not an optimization — it is what makes large catalogue batches feasible.
...
UA = "OpenSourceRail-design/0.2 (+https://github.com/modernecotech/OpenSourceRail)"

# Per-mirror timeout. Overpass can legitimately take 30-90s on a busy city,
# but waiting longer blocks every uncached city behind one unhealthy mirror.
HTTP_TIMEOUT_S = 90
...
def _fetch_overpass_tiled(
    ...
    *,
    ...
    """Fetch four smaller bbox queries and merge duplicate OSM elements.

    Large city queries can time out on every public Overpass mirror even when
    each quadrant is inexpensive. This bounded fallback keeps the canonical
    tag query intact while reducing server-side scan and response sizes.
    """
    ...
    for index, tile in enumerate(tiles, start=1):
        log.info(
            "overpass tiled fallback: depth %d quadrant %d/%d",
            depth + 1,
            index,
            len(tiles),
        )
        query = _build_query(tile)
        tile_slug = f"{slug}-tile-{depth + 1}-{index}"
        tile_cache = _cache_path(cache_root, query, tile_slug)
        try:
            payload = json.loads(tile_cache.read_text())
            log.info("tile cache hit: %s", tile_cache.name)
            ...
            try:
                ...
                if depth >= 2:
                    raise
                payload = _fetch_overpass_tiled(
                    tile,
                    cache_root,
                    tile_slug,
                    depth=depth + 1,
                )
            tmp = tile_cache.with_suffix(tile_cache.suffix + ".part")
            tmp.write_text(json.dumps(payload))
            tmp.replace(tile_cache)
...
def fetch_city(
    ...
    whole_failure = cpath.with_suffix(cpath.suffix + ".whole-failed")
    recent_whole_failure = (
        whole_failure.exists()
        and time.time() - whole_failure.stat().st_mtime < 3600
    )
    if recent_whole_failure:
        log.info("recent whole-city failure cached; resuming tiled fetch")
        raw = _fetch_overpass_tiled(bbox, cache_root, slug)
    else:
        try:
            raw = _fetch_overpass(query)
            whole_failure.unlink(missing_ok=True)
        except OverpassError:
            whole_failure.touch()
            log.warning("whole-city Overpass request failed; retrying as four quadrants")
            raw = _fetch_overpass_tiled(bbox, cache_root, slug)
