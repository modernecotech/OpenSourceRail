...
try {
 ...
 integration('stop');integrationStopped=true;
 ...
 integration('start');integrationStopped=false;
 await expect(page.getByText('180',{exact:true})).toBeVisible({timeout:45000});
 console.log('PASS gateway restart restores current supervised values without replaying a command');
} finally {fs.writeFileSync(controls,'{}');if(integrationStopped)integration('start');await browser.close();}
