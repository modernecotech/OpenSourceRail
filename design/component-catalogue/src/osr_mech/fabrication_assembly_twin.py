...
def twin_checks(streams: tuple[FabricationStream, ...] | None = None) -> tuple[dict[str, Any], ...]:
    ...
    checks = (
        ...
        ("train-product-tree-loaded", len(train_manifest["product_items"]) == 120 and len(train_manifest["assemblies"]) == 26,
         {"items": len(train_manifest["product_items"]), "assemblies": len(train_manifest["assemblies"])}),
        ("train-product-tree-fully-animated", trainset_assembly_graph()["animated_node_count"] == 146,
