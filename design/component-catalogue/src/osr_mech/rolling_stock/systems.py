...
from .baseline import PROMOTED_ROOF_SOLAR_MODULES_PER_CAR
...
ROOF_SOLAR_MODULES_PER_CAR = PROMOTED_ROOF_SOLAR_MODULES_PER_CAR
...
def battery_pack_set(dims: CarDimensions = CarDimensions()) -> Compound:
        ...
        for x in x_offsets:
            ...
            parts.append(
                _box(
                    ...
                    70.0,
                    (x + 520.0, y, z + 220.0),
                    "Battery off-gas and temperature detector",
                    COLOR_ACCESS,
                )
            )
            parts.append(
    ...
    parts.extend(
        [
            _box(620.0, 360.0, 420.0, (1050.0, 0.0, 620.0), "Per-car water-mist reservoir", Color(0.20, 0.55, 0.85)),
            _cyl(85.0, 260.0, (1450.0, 0.0, 620.0), "Monitored DC water-mist pump", Color(0.20, 0.55, 0.85)),
            _box(13_000.0, 28.0, 28.0, (0.0, 0.0, 940.0), "Stainless battery mist distribution pipe", COLOR_METAL),
        ]
    )
...
def traction_power_rack() -> Compound:
    """Per-car 800 V-class power rack with two motor controllers."""

    parts: list[Part] = []
    for y in (-420.0, 420.0):
        parts.append(
            _part(
                Box(620.0, 360.0, 260.0).locate(Location((-1650.0, y, -330.0))),
    ...
    parts.extend(
        [
            _box(820.0, 520.0, 380.0, (1650.0, 0.0, -340.0), "Isolated 800 V to 110/48/24 V DC/DC rack", COLOR_ELECTRONICS),
            _box(780.0, 360.0, 320.0, (0.0, 0.0, -370.0), "Monitored traction/battery coolant pump and manifold", Color(0.12, 0.45, 0.62)),
            _box(520.0, 300.0, 220.0, (-350.0, -900.0, -300.0), "Per-car roof PV MPPT DC/DC", COLOR_ELECTRONICS),
            _box(620.0, 340.0, 260.0, (350.0, -900.0, -300.0), "Station DC protection, isolation monitoring, contactors and precharge", COLOR_HV),
            _box(780.0, 90.0, 58.0, (0.0, -1188.0, -470.0), "650–700 V car DC-link busbar", COLOR_HV),
        ]
    )
    for x in (-260.0, -90.0, 90.0, 260.0):
    ...
    parts.extend(
        [
            _box(1180.0, 90.0, 72.0, (0.0, -1440.0, 365.0), "Station-charge HV route to protected car DC link", COLOR_HV),
            _box(90.0, 120.0, 1250.0, (-620.0, -760.0, 560.0), "Removable roof PV downlink to per-car MPPT", COLOR_HV),
        ]
    )
    return Compound(label="800 V-class traction, DC auxiliary, and charging assembly", children=parts)
