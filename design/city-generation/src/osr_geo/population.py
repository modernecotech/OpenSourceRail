...
MAX_WORLDPOP_RASTER_BYTES = 1_000_000_000


class _RasterTooLarge(RuntimeError):
    pass
...
def fetch_population_raster(country: str, cache_dir: Path) -> Path | None:
    """Fetch once per country even when several city workers run in parallel."""
    ...
    iso3l = iso3.lower()
    lock_path = cache_dir / f".{iso3l}.worldpop.lock"
    unavailable_path = cache_dir / f".{iso3l}.worldpop-unavailable"
    with lock_path.open("a", encoding="utf-8") as lock:
        fcntl.flock(lock.fileno(), fcntl.LOCK_EX)
        if unavailable_path.exists() and time.time() - unavailable_path.stat().st_mtime < 3600:
            return None
        result = _fetch_population_raster_unlocked(country, cache_dir)
        if result is None:
...
def _fetch_population_raster_unlocked(country: str, cache_dir: Path) -> Path | None:
    ...
    for path_template, suffix in _WORLDPOP_VARIANTS:
        ...
        # Large WorldPop country rasters occasionally end with an incomplete
        # transfer. Keep the partial file and request only the missing range;
        # restart cleanly if the server ignores Range. A 404 is permanent for
        # that variant, so move immediately to the next URL.
        for attempt in range(1, 4):
            try:
                ...
                with urllib.request.urlopen(request, timeout=120) as response:
                    ...
                    if total_size > MAX_WORLDPOP_RASTER_BYTES:
                        raise _RasterTooLarge(
                            f"country raster is {total_size / 1_000_000:.0f} MB"
                        )
                    if offset and status != 206:
                        if offset > 100_000_000:
                            raise _RasterTooLarge(
                                "server cannot resume a population raster larger than 100 MB"
                            )
                    ...
                    with tmp.open(mode) as handle:
                        while chunk := response.read(1024 * 1024):
                            handle.write(chunk)
                            if handle.tell() > MAX_WORLDPOP_RASTER_BYTES:
                                raise _RasterTooLarge(
                                    "country raster exceeded the 1 GB optional-layer cap"
                                )
                        handle.flush()
                        os.fsync(handle.fileno())
                # Keep download integrity independent of optional rasterio so
                # the fetch/cache layer also works in the lightweight planner
                # environment. Validate the TIFF signature and require a
                # substantive country raster; sampling performs the deeper
                # rasterio validation when that optional dependency is present.
                if tmp.stat().st_size < 1_000_000:
                    raise RuntimeError("downloaded raster is unexpectedly small")
                with tmp.open("rb") as handle:
                    signature = handle.read(4)
                if signature not in (b"II*\x00", b"MM\x00*"):
                    raise RuntimeError("downloaded population file is not a TIFF")
                tmp.replace(out)
                return out
                ...
                if error.code == 404:
                    tmp.unlink(missing_ok=True)
                    break
            except _RasterTooLarge as error:
                last_error = error
                tmp.unlink(missing_ok=True)
                break
                ...
                last_error = error
            log.warning(
                "WorldPop %s attempt %d/3 incomplete (%s); retrying",
                iso3,
                attempt,
                last_error,
            )
        ...
        log.warning(
            ...
            iso3, suffix.lstrip("_"), last_error,
        )
        tmp.unlink(missing_ok=True)
