    ...
    def _fleets_section(self) -> str:
        ...
        for f in self.design.get("fleets", []):
            ...
            if f.get("station_stabling", False):
                ...
                out.append(f"spare_count = {int(f.get('spare_count', 0))}\n")
                out.append(f"cold_reserve_count = {int(f.get('cold_reserve_count', 0))}\n")
...
def generate_from_path(
    ...
    if templates_root is None:
        ...
        # (post-reorg; the old `cities/catalogue/templates/` is kept as a
        # fallback so any in-flight branch still resolves).
        cur = design_path.parent
        for _ in range(10):
            for rel in ("lib/templates", "templates", "cities/catalogue/templates"):
                candidate = cur / rel
