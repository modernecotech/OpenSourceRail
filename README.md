...
| **Software in the loop** | One PTP-synchronised deterministic run preflights all five deployment roles, then connects train, station, energy, declared point/crossing, wayside and regen controllers through T2G and the track-state codec to depot services and OCC; signed fares, conservation and dispatch holds remain auditable. |
...
| **Campaign-ready city catalogue** | Deterministic [developing-world government, municipality, funder, university, research, nonprofit and media outreach packages](marketing/README.md) turn each in-scope design into a concise brief, audience-specific email, image set and contact-research queue without guessing addresses or sending messages. |
...
| **Deterministic browser acceptance** | Pinned Playwright tests build both Rust/WASM consoles, verify onboard/infrastructure/backend evidence, exercise portal persistence, and run 111 City Studio edit, adapter, restart and persistence checks in CI. |
...
per wayside section. TCMS trips inhibit dispatch or section progress on the
next control cycle. Generated scenarios carry a mechanical system contract;
