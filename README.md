...
| Buildable modular trainset | The controlled [`LM3-FA-001` first-article baseline](design/component-catalogue/catalog/buildable-trainset/first-article-baseline.json) separates 120 product rows into 26 part→subassembly→car→trainset nodes, including discrete chassis/body modules, front glass/lamp fitout, roof/HVAC/PV fairings, interior service panels, recovery interfaces and finish kits. Git includes [120 separate native FreeCAD parts and 26 tested assemblies](design/component-catalogue/models/cad/README.md), matching [split IFC4.3 part/assembly files](engineering/models/bim/reference/README.md), 619 geometric primitives, installed-coordinate/configuration-state evidence, nine timed methods, 30 tooling/mould families and shop travelers. The 62 locally made rows have controlled geometry and manufacturing-method routes; bought-in rows retain supplier qualification and local-equivalent gates. The [model coverage register](engineering/models/model-coverage.md) separates envelopes, interface detail and release maturity, while unresolved supplier/test evidence remains in the [public work packages](design/component-catalogue/catalog/buildable-trainset/first-article-work-packages.md). |
