...
Shared templates and city-specific profiles make the operating platform reproducible across the catalogue. The installed **Samawah and Mosul simulation pilots** each link a station and a trainset to native Rust energy, station SCADA, BMS, auxiliary-power, HVAC and condition-monitoring evaluators. Source-quality checks, controller state retained between samples, alarm delays and a durable delivery queue connect controller conditions to ERP maintenance cases. An authorised maintainer can review the linked Asset, part/warehouse availability, technician and expected downtime before creating one repeat-safe native Asset Repair; ERP submission then records actual part consumption and downtime. Clearing a condition or closing/completing its business records still cannot release a railway asset. Prepared supervisory revisions now produce a review-bound design/embedded/lifecycle impact report before replacement; FUXA project replacement is separately bound to a manifest of desired city packages, revisions, devices and displays plus explicit live additions/changes/removals. Both paths reject stale review. The generated [operating-readiness audit](docs/operating/readiness.md) cross-checks every city's design, scenario, asset register, operations manifest and project twin; compiles all **266 ERP profiles, 266 component packages and 266 real-asset supervision packages**; and records the exact remaining boundary. Samawah and Mosul carry full local ERP task payloads, while the other 264 large payloads are regenerated on demand. Legal companies, approved calendars, ERP masters and physical controller bindings remain operator/commissioning inputs.
