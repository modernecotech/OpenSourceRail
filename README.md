...
| Business execution and embedded maintenance | ERPNext handles projects, tasks, procurement, stock, manufacturing, quality, finance, HR and maintenance. Nine native Rust evaluators feed station/vehicle/wayside supervision, timestamped history and deduplicated ERP maintenance cases through the [integration gateway](docs/lifecycle/embedded-integration.md). A reviewed case transition creates a native Asset Repair with parts, technician assignment, expected/actual downtime and linked condition/configuration evidence. |
...
| Deterministic integrated testing | Pinned Playwright acceptance verifies the browser applications, adapters, engineering jobs and restart persistence; a disposable CI deployment additionally exercises native ERPNext, FUXA, the gateway, embedded controller bridge and Workbench through outage and recovery. |
...
Shared templates and city-specific profiles make the operating platform reproducible across the catalogue. The installed **Samawah and Mosul simulation pilots** each link a station, trainset and real switch to nine native Rust energy, station, vehicle, points, crossing and fare evaluators; crossings remain dormant until real asset identities exist, and fare gates are station aggregates. Source-quality checks, retained controller state, alarm delays and a durable queue connect controller conditions to ERP maintenance cases. An authorised maintainer can review the linked Asset, parts, warehouse, technician and downtime before creating one repeat-safe native Asset Repair; ERP submission records actual consumption and downtime. Neither business closure nor the read-only wayside views can release an asset, command points/barriers or grant movement authority. Prepared supervisory revisions produce a review-bound design/embedded/lifecycle impact report; FUXA replacement is separately bound to desired packages, revisions, devices/displays and explicit live changes. Both reject stale review. The generated [operating-readiness audit](docs/operating/readiness.md) checks every city's design, scenario, asset register, operations manifest and project twin; compiles all **266 ERP profiles, 266 component packages and 266 real-asset supervision packages**; and records the remaining boundary. Samawah and Mosul carry full ERP task payloads; the other 264 are regenerated on demand. Legal companies, approved calendars, ERP masters and physical bindings remain operator/commissioning inputs.

The UI includes city-filtered ERP workflows, searchable equipment, measurements and trends, prepared change impact, reviewed lifecycle evidence, serial/batch replacement history and bounded simulation lighting requests. ERP/FUXA retain their own sign-in and permissions. Desktop editors are linked rather than browser-hosted; physical supplier interfaces and commissioning remain deployment work. Start with the [Workbench guide](docs/workbench/README.md), [ERPNext city setup](docs/operating/city-platform.md), [FUXA/gateway setup](deployment/supervision/README.md) and [crate integration contract](docs/lifecycle/embedded-integration.md).
