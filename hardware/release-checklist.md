...
- **Custom-board fabrication release:** OSR-specific carrier, power,
  ...
  [RFC 0007](../docs/rfcs/0007-hardware-reference-designs.md). This
...
- `hardware/<class>/diy-assembly/README.md` with assembly order and
  role-specific commissioning steps.
- `hardware/<class>/diy-assembly/bom-vN.csv` with exact SKUs,
  alternates, lifecycle status, supplier links, and substitution
  rules.
- `hardware/<class>/diy-assembly/wiring-map-vN.md` with terminal
  blocks, connector pinouts, cable labels, ferrule sizes, and harness
  routing.
- `hardware/<class>/diy-assembly/enclosure-vN.md` with DIN-rail,
  cabinet, sensor-mount, ingress-protection, and service-clearance
  notes.
- `hardware/<class>/diy-assembly/power-thermal-vN.md` with input
  ranges, fuse ratings, rail loads, heat budget, and derating limits.
- `hardware/<class>/bring-up/vN.md` with SD-card image checksum,
  boot log, `osr-selftest` output, smoke-test results, and known
  issues.
- `hardware/<class>/evidence/vN/` with photos, oscilloscope captures
