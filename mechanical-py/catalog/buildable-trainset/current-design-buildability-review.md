...
| `BDR-008` | `yellow` | joint and fastener control | Every assembly integration step now carries machine-readable join classes, torque authority, and release status; numerical torque, locking, and re-torque values remain open until supplier instructions or joint calculations are released. | Close the generated joint-control schedule by joint ID and reference the accepted values from interface drawings and shop travelers. |
