...
AT_GRADE_PANEL_WIDTH_MM = 6200.0
AT_GRADE_BASE_THICKNESS_MM = 280.0
AT_GRADE_PLINTH_WIDTH_MM = 420.0
AT_GRADE_PLINTH_HEIGHT_MM = 220.0
...
def at_grade_rail_y_positions() -> tuple[float, float, float, float]:
    """Rail Y positions for the double-track at-grade slab."""

    half_track = TRACK_CENTRE_SPACING_MM / 2.0
    half_gauge = STANDARD_GAUGE_MM / 2.0
    return (
        -half_track - half_gauge,
        -half_track + half_gauge,
        half_track - half_gauge,
        half_track + half_gauge,
    )
...
def at_grade_concrete_volume_m3(length_mm: float = PANEL_LENGTH_MM) -> float:
    """Nominal concrete volume for one double-track at-grade slab panel."""
...
def at_grade_slab_panel(length_mm: float = PANEL_LENGTH_MM) -> Compound:
    """Reference 6 m double-track ballastless at-grade slab panel."""
    ...
    return Compound(label=f"At-grade ballastless slab panel ({length_mm:.0f} mm)", children=parts)
