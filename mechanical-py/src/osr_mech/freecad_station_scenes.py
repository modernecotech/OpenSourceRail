...
CAPTURES: tuple[Capture, ...] = (
    Capture("At-grade side-platform station", "freecad-at-grade-station-track-train.png"),
    Capture("Elevated side-platform station", "freecad-elevated-station-track-train.png"),
    Capture("Elevated interchange station", "freecad-elevated-interchange-track-train.png", 2200, 1450),
)
...
def _build_elevated_scene(doc):
    group = _group(doc, "Elevated side-platform station")
    _ground(doc, group, 86, 38)
...
def _build_interchange_scene(doc):
    group = _group(doc, "Elevated interchange station")
...
def _show_group(doc, group_label: str) -> None:
    _hide_all(doc)
    for obj in doc.Objects:
        if getattr(obj, "Label", "") == group_label:
