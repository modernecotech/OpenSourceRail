...
def _clean_freecad_shapes(shapes):
    """Fuse overlapping envelope pieces into FEM/review-ready solids.

    The Python facade intentionally preserves child parts and summed volume
    for catalogue tests.  Native FreeCAD handoff documents, however, need
    an OCC shape without coincident/intersecting child faces.  Fuse one piece
    at a time so a single troublesome envelope does not discard the rest.
    """
    if not shapes:
        return None
    result = _copy_freecad_shape(shapes[0])
    for shape in shapes[1:]:
        try:
            result = result.fuse(shape)
        except Exception:
            result = FreeCADPart.makeCompound([result, shape])
    try:
...
def to_freecad_shape(obj: Part | Compound, *, clean: bool = False):
    """Return a native FreeCAD shape for a catalogue object, if possible.

    ``clean=True`` is used for generated FreeCAD documents and converts
    overlapping envelope children into a fused OCC result.
    """

    if isinstance(obj, Compound):
        if obj.wrapped is not None and not clean:
            return _copy_freecad_shape(obj.wrapped)
        if FreeCADPart is None:
            return None
        child_shapes = [to_freecad_shape(child, clean=clean) for child in obj.children]
        child_shapes = [shape for shape in child_shapes if shape is not None]
        if not child_shapes:
            return None
        return _clean_freecad_shapes(child_shapes) if clean else FreeCADPart.makeCompound(child_shapes)
