...
- halt:        ceil(platform_length / 6.0 m). halt platform is 0.4×
               consist length, so the canopy is short — no shelter
               for rear cars.
...
def station_canopy(
    ...
    not include the platform L-units — those are in `osr_mech.civil`.
    """
...
def canopy_steel_mass_kg(
    ...
    # + 1 brace (~2500 mm × 18.0 kg/m) + 5 % gussets/plates.
    per_bay_kg = (3.2 * 42.3) + (3.5 * 35.5) + (2.5 * 18.0)
    per_bay_kg *= 1.05
    return bays * per_bay_kg
